...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name_suffix', default='Jan1', type=str,
    ...
    checkpoint_filename_out = os.path.join(model_params["training_parameters"]['model_checkpoint_location'],
                                           model_name + "_final")

    if args.skip_existing:
        if os.path.isfile(checkpoint_filename_out):
            print("Model already exists. Skipping training without error.")
            sys.exit(0)
    else:
        if os.path.isfile(checkpoint_filename_out):
    ...
    model.save(model_checkpoint=checkpoint_filename_out,
               encoder_parameters=model_params["encoder_parameters"],
               decoder_parameters=model_params["decoder_parameters"],
