...
def plot_histograms(
    all_evol_indices, dict_models, dict_pathogenic_cluster_index, protein_GMM_weight, plot_location, output_eve_scores_filename_suffix, protein_list
):
    ...
    if protein_GMM_weight > 0.0:
        for protein in tqdm.tqdm(protein_list, "Plot protein histograms"):
            ...
            plt.plot(x, pdf_benign, "--k", color="xkcd:sky blue", linewidth=4)
            plt.hist(
                all_evol_indices["evol_indices"][all_evol_indices["protein_name"] == protein],
                color="xkcd:grey",
                bins=80,
                histtype="stepfilled",
                alpha=0.4,
            ...
            plt.yticks(fontsize=10)
            plt.savefig(
                plot_location + os.sep + "histogram_random_samples_" + str(output_eve_scores_filename_suffix) + "_" + str(protein) + ".png",
                dpi=800,
                bbox_inches="tight",
            )
            plt.clf()
