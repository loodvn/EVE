...
def compute_uncertainty_deciles(score_dataframe, score_name="EVE_scores", uncertainty_name="uncertainty", suffix=""):
    ...
    for decile in range(1, 11):
        ...
        scores_at_uncertainty_deciles_cuttoffs_UB_lower_part[str(decile)] = np.max(
            score_dataframe[score_name][(score_dataframe[uncertainty_deciles_name] == decile) & (score_dataframe[score_name] < 0.5)]
        )
        scores_at_uncertainty_deciles_cuttoffs_LB_upper_part[str(decile)] = np.min(
            score_dataframe[score_name][(score_dataframe[uncertainty_deciles_name] == decile) & (score_dataframe[score_name] > 0.5)]
        )
...
def compute_uncertainty_quartiles(score_dataframe, score_name="EVE_scores", uncertainty_name="uncertainty", suffix=""):
    ...
    for quartile in range(1, 5):
        ...
        scores_at_uncertainty_quartiles_cuttoffs_UB_lower_part[str(quartile)] = np.max(
            score_dataframe[score_name][(score_dataframe[uncertainty_deciles_name] == quartile) & (score_dataframe[score_name] < 0.5)]
        )
        scores_at_uncertainty_quartiles_cuttoffs_LB_upper_part[str(quartile)] = np.min(
            score_dataframe[score_name][(score_dataframe[uncertainty_deciles_name] == quartile) & (score_dataframe[score_name] > 0.5)]
        )
...
def compute_performance_by_uncertainty_decile(
    score_dataframe,
    metric="Accuracy",
    verbose=False,
    ...
    class_100pct_retained_name="EVE_classes_100_pct_retained",
    suffix="",
):
    (
        uncertainty_cutoffs_deciles,
        scores_at_uncertainty_deciles_cuttoffs_UB_lower_part,
        scores_at_uncertainty_deciles_cuttoffs_LB_upper_part,
    ) = compute_uncertainty_deciles(score_dataframe, score_name, uncertainty_name, suffix)
    ...
    for decile in range(1, 11):
        ...
        if metric == "Accuracy":
            ...
            performance_decile = compute_avg_protein_level_AUC_with_uncertain(
                scores=score_dataframe[score_name],
                class_pred=score_dataframe[classification_name],
                labels=score_dataframe[label_name],
                protein_ID=score_dataframe[protein_name],
            )
        performance_by_uncertainty_deciles[decile] = performance_decile
        pathogenic_rate_by_uncertainty_deciles[decile] = compute_pathogenic_rate_with_uncertain(
            class_pred=score_dataframe[classification_name], labels=score_dataframe[label_name]
        )
        if verbose:
            print(
                str(metric)
                + " when dropping the "
                + str((10 - decile) * 10)
                + "% of cases with highest uncertainty:\t"
                + str(performance_by_uncertainty_deciles[decile])
                + "% \t with pathogenic rate of "
                + str(pathogenic_rate_by_uncertainty_deciles[decile])
                + "%\n"
            )
...
def compute_performance_by_uncertainty_quartile(
    score_dataframe,
    metric="Accuracy",
    verbose=False,
    score_name="EVE_scores",
    ...
    (
        ...
        scores_at_uncertainty_quartiles_cuttoffs_UB_lower_part,
        scores_at_uncertainty_quartiles_cuttoffs_LB_upper_part,
    ) = compute_uncertainty_quartiles(score_dataframe, score_name, uncertainty_name, suffix)
    ...
    for quartile in range(1, 5):
        ...
        if metric == "Accuracy":
            ...
            performance_quartile = compute_avg_protein_level_AUC_with_uncertain(
                scores=score_dataframe[score_name],
                class_pred=score_dataframe[classification_name],
                labels=score_dataframe[label_name],
                protein_ID=score_dataframe[protein_name],
        ...
        pathogenic_rate_by_uncertainty_quartiles[quartile] = compute_pathogenic_rate_with_uncertain(
            class_pred=score_dataframe[classification_name], labels=score_dataframe[label_name]
        )
        if verbose:
            print(
                ...
                + str((4 - quartile) * 25)
                + "% of cases with highest uncertainty:\t"
                + str(performance_by_uncertainty_quartiles[quartile])
                + "% \t with pathogenic rate of "
                + str(pathogenic_rate_by_uncertainty_quartiles[quartile])
                + "%\n"
            )
            print("Uncertainty quartile #" + str(quartile) + " cutoff: " + str(uncertainty_cutoffs_quartiles[str(quartile)]) + "\n")
            print(
                "Score upper bound for lower part in uncertainty quartile: " + str(scores_at_uncertainty_quartiles_cuttoffs_UB_lower_part[str(quartile)]) + "\n"
            )
            print(
                "Score lower bound for higher part in uncertainty quartile: "
                + str(scores_at_uncertainty_quartiles_cuttoffs_LB_upper_part[str(quartile)])
                + "\n"
            )
...
def compute_weighted_score_two_GMMs(X_pred, main_model, protein_model, cluster_index_main, cluster_index_protein, protein_weight):
    return protein_model.predict_proba(X_pred)[:, cluster_index_protein] * protein_weight + (main_model.predict_proba(X_pred)[:, cluster_index_main]) * (
        1 - protein_weight
    )
...
def compute_weighted_class_two_GMMs(X_pred, main_model, protein_model, cluster_index_main, cluster_index_protein, protein_weight):
    ...
    proba_pathogenic = protein_model.predict_proba(X_pred)[:, cluster_index_protein] * protein_weight + (
        main_model.predict_proba(X_pred)[:, cluster_index_main]
    ) * (1 - protein_weight)
