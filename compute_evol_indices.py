...
if __name__ == "__main__":
    ...
    list_valid_mutations, evol_indices, _, _ = model.compute_evol_indices(
        msa_data=data, list_mutations_location=args.mutations_location, num_samples=args.num_samples_compute_evol_indices, batch_size=args.batch_size
    )
    ...
    evol_indices_output_filename = (
        ...
        + protein_name
        + "_"
        + str(args.num_samples_compute_evol_indices)
        + "_samples"
        + args.output_evol_indices_filename_suffix
        + ".csv"
    )
    try:
