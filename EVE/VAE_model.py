...
class VAE_model(nn.Module):
    ...
    def __init__(self, model_name, data, encoder_parameters, decoder_parameters, random_seed):
    ...
    def train_model(self, data, training_parameters):
        ...
        if training_parameters["use_lr_scheduler"]:
            scheduler = optim.lr_scheduler.StepLR(
                optimizer, step_size=training_parameters["lr_scheduler_step_size"], gamma=training_parameters["lr_scheduler_gamma"]
            )

        if training_parameters["use_validation_set"]:
            x_train, x_val, weights_train, weights_val = train_test_split(
                data.one_hot_encoding, data.weights, test_size=training_parameters["validation_set_pct"], random_state=self.random_seed
            )
        ...
        for training_step in tqdm.tqdm(range(1, training_parameters["num_training_steps"] + 1), desc="Training model"):
            ...
            if training_step % training_parameters["log_training_freq"] == 0:
                progress = (
                    "|Train : Update {0}. Negative ELBO : {1:.3f}, BCE: {2:.3f}, KLD_latent: {3:.3f}, KLD_decoder_params_norm: {4:.3f}, Time: {5:.2f} |".format(
            ...
            if training_step % training_parameters["save_model_params_freq"] == 0:
                self.save(
                    model_checkpoint=training_parameters["model_checkpoint_location"] + os.sep + self.model_name + "_step_" + str(training_step),
            ...
            if training_parameters["use_validation_set"] and training_step % training_parameters["validation_freq"] == 0:
                ...
                progress_val = "\t\t\t|Val : Update {0}. Negative ELBO : {1:.3f}, BCE: {2:.3f}, KLD_latent: {3:.3f}, KLD_decoder_params_norm: {4:.3f}, Time: {5:.2f} |".format(
                    training_step, val_neg_ELBO, val_BCE, val_KLD_latent, val_KLD_global_parameters, time.time() - start
                )
                print(progress_val)
                if training_parameters["log_training_info"]:
                    with open(filename, "a") as logs:
                ...
                if val_neg_ELBO < best_val_loss:
                    ...
                    self.save(
                        model_checkpoint=training_parameters["model_checkpoint_location"] + os.sep + self.model_name + "_best",
                        encoder_parameters=self.encoder_parameters,
                        decoder_parameters=self.decoder_parameters,
                        training_parameters=training_parameters,
                    )
    ...
    def test_model(self, x_val, weights_val, batch_size):
        ...
        with torch.no_grad():
            ...
            neg_ELBO, BCE, KLD_latent, KLD_global_parameters = self.loss_function(
                recon_x_log, x, mu, log_var, kl_latent_scale=1.0, kl_global_params_scale=1.0, annealing_warm_up=0, training_step=1, Neff=self.Neff_training
