...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--load_GMM_models", default=False, action="store_true", help="If True, load GMM model parameters. If False, train GMMs from evol indices files"
    )
    parser.add_argument(
        "--GMM_parameter_location", default=None, type=str, help="Folder where GMM objects are stored if loading / to be stored if we are re-training"
    )
    ...
    parser.add_argument(
        "--compute_EVE_scores", default=False, action="store_true", help="Computes EVE scores and uncertainty metrics for all input protein mutations"
    )
    parser.add_argument(
    ...
    all_evol_indices = pd.concat(
        [
            pd.read_csv(args.input_evol_indices_location + os.sep + protein + args.input_evol_indices_filename_suffix + ".csv", low_memory=False)[
                list_variables_to_keep
            ]
            for protein in protein_list
            if os.path.exists(args.input_evol_indices_location + os.sep + protein + args.input_evol_indices_filename_suffix + ".csv")
        ],
        ignore_index=True,
    )
    ...
    if args.load_GMM_models:
        ...
        dict_pathogenic_cluster_index = pickle.load(
            open(args.GMM_parameter_location + os.sep + "GMM_pathogenic_cluster_index_dictionary_" + args.GMM_parameter_filename_suffix, "rb")
        )
    else:
        dict_models = {}
        ...
        with open(GMM_stats_log_location, "a") as logs:
            logs.write(
                ",".join(
                    ...
                    for x in [
                        "main",
                        np.array(main_GMM.weights_).flatten()[dict_pathogenic_cluster_index["main"]],
                        np.array(main_GMM.means_).flatten()[dict_pathogenic_cluster_index["main"]],
                        np.array(main_GMM.means_).flatten()[1 - dict_pathogenic_cluster_index["main"]],
                        np.sqrt(np.array(main_GMM.covariances_).flatten()[dict_pathogenic_cluster_index["main"]]),
                    ...
                    with open(GMM_stats_log_location, "a") as logs:
                        logs.write(
                            ",".join(
                                ...
                                for x in [
                                    protein,
                                    np.array(protein_GMM.weights_).flatten()[dict_pathogenic_cluster_index[protein]],
                                    np.array(protein_GMM.means_).flatten()[dict_pathogenic_cluster_index[protein]],
                                    np.array(protein_GMM.means_).flatten()[1 - dict_pathogenic_cluster_index[protein]],
                                    np.sqrt(np.array(protein_GMM.covariances_).flatten()[dict_pathogenic_cluster_index[protein]]),
    ...
    if args.plot_histograms:
        ...
        plot_helpers.plot_histograms(
            ...
            dict_models,
            dict_pathogenic_cluster_index,
            args.protein_GMM_weight,
            args.plot_location + os.sep + "plots_histograms" + os.sep + args.output_eve_scores_filename_suffix,
            args.output_eve_scores_filename_suffix,
            protein_list,
        )
    ...
    if args.compute_EVE_scores:
        if args.protein_GMM_weight > 0.0:
            ...
            all_scores["EVE_classes_100_pct_retained"] = np.array(
                pd.Series(gmm_class).map(lambda x: "Pathogenic" if x == dict_pathogenic_cluster_index["main"] else "Benign")
            )
    ...
    if args.plot_scores_vs_labels:
        ...
        all_scores_mutations_with_labels = pd.merge(
            all_scores, labels_dataset[["protein_name", "mutations", "ClinVar_labels"]], how="inner", on=["protein_name", "mutations"]
        )
